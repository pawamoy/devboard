...
*Devboard Insiders* is a compatible drop-in replacement for *Devboard*,
...
*Devboard Insiders* can be installed with `pip` [using SSH][using ssh]:
...
Self-hosting the Insiders package makes it possible to depend on *Devboard* normally,
...
with [Twine]:
  ...
  [Github Releases]: https://github.com/pawamoy-insiders/devboard/releases
...
>
> Now when running `pip install devboard`,
...
Of course, you can use *Devboard Insiders* directly from `git`:
...
When upgrading Insiders, you should always check the version of *Devboard*
