...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("devboard", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc devboard",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/devboard" in origin:
            ctx.run("git remote add upstream git@github.com:pawamoy/devboard", silent=True, nofail=True)
            ctx.run(
